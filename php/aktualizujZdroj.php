<?php
  ...
  $bakaUcetID=(int) $_GET['bakaUcetID'];
  $zdroj=$funkce->mysqli->query('SELECT COUNT(*),url,prihlJmeno,heslo FROM bakaUcty WHERE ucetID='.$_SESSION['ucetID'].' AND bakaUcetID='.$bakaUcetID)->fetch_assoc();
  if($zdroj['COUNT(*)']!=1) exit(JSON_encode(array('state'=>'unauthorised')));
  ...
  $ch=curl_init($zdroj['url'].'login.aspx');
  ...
  curl_setopt($ch,CURLOPT_REFERER,$zdroj['url'].'login.aspx');
  curl_setopt($ch,CURLOPT_POST,true);
  $postdata='__LASTFOCUS=&__EVENTTARGET=&__EVENTARGUMENT=&'.urlencode($prihlInput).'='.$zdroj['prihlJmeno'].'&ctl00%24cphmain%24TextBoxHeslo='.$zdroj['heslo'].'&ctl00%24cphmain%24ButtonPrihlas=&DXScript=1_44%2C1_76%2C2_27';
  ...
  curl_setopt($ch,CURLOPT_REFERER,$zdroj['url'].'login.aspx');
  curl_setopt($ch,CURLOPT_URL,$zdroj['url'].'uvod.aspx');
  ...
  curl_setopt($ch,CURLOPT_REFERER,$zdroj['url'].'uvod.aspx');
  curl_setopt($ch,CURLOPT_URL,$zdroj['url'].$znamkyURL);
  ...
  curl_setopt($ch,CURLOPT_URL,$zdroj['url'].'prehled.aspx?s=4');
  ...
  foreach($predmety as $predmet) {     // výpis předmětů do databáze
    ...
    $r=$funkce->mysqli->query('SELECT predmetID FROM bakaPredmety WHERE nazev="'.$funkce->mysqli->real_escape_string($nazev).'" AND bakaUcetID="'.$bakaUcetID.'"');
    if($r->num_rows<1) {
      $funkce->mysqli->query('INSERT INTO bakaPredmety (nazev,ucitel,prumer,ctvrtleti,pololeti,bakaUcetID) VALUES ("'.$funkce->mysqli->real_escape_string($nazev).'","","'.$funkce->mysqli->real_escape_string($prumer).'",'.$ctvrtletiDb.','.$pololetiDb.','.$bakaUcetID.')');  // uložení nového předmětu
  ...
  curl_setopt($ch,CURLOPT_URL,$zdroj['url'].'prehled.aspx?s=5');
  ...
  if($vychOpatreni) foreach($vychOpatreni as $c=>$opatreni) {
    ...
    $a=$funkce->mysqli->query('SELECT COUNT(*),opatreniID FROM bakaVychOpatreni WHERE poradi='.$c.' AND bakaUcetID='.$bakaUcetID)->fetch_assoc();
    if($a['COUNT(*)']!=1) {
      $funkce->mysqli->query('INSERT INTO bakaVychOpatreni (poradi,druh,datum,text,bakaUcetID) VALUES ('.$c.','.$druh.','.$datum.','.$text.','.$bakaUcetID.')');  // uložení výchovného opatření
      }
    else {
  ...
  $funkce->mysqli->query('UPDATE bakaUcty SET skola="'.$funkce->mysqli->real_escape_string($skola).'", jmeno="'.$funkce->mysqli->real_escape_string($jmeno).'", nove='.$noveZnamkyC.', posledniKontrola='.time().', chyba=NULL WHERE bakaUcetID='.$bakaUcetID);  // aktualizace údajů účtu
  $funkce->mysqli->query('COMMIT');
  curl_close($ch);

  $json['state']='success';
  $json['nove']=$noveZnamkyC;
